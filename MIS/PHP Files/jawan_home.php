...
// Fetch leave details for the jawan
...
if (mysqli_num_rows($result_leave) > 0) {
    $row = mysqli_fetch_assoc($result_leave);
    $start = $row['date_from'];
    $cur_date = date("Y-m-d");

    // Check if the leave start date is today or earlier
    if ($cur_date == $start) {
        // Update jawan status to inactive
        $sql_update_status = "UPDATE jawaan_details SET jawaan_status = 'inactive' WHERE jawaan_id = '$id'";
